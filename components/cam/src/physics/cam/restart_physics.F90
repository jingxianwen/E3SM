module restart_physics
  ...
  use radiation,          only: cosp_cnt_init, cosp_cnt, rad_randn_seedrst, kiss_seed_num, flag_emis  
                                ! added flag_emis by U-MICH team, Mar 19, 2020.
...
!
    ...
    !type(var_desc_t) :: do_emis_desc, ts_atm_desc, srf_emis_spec_desc
    type(var_desc_t) :: ts_atm_desc, srf_emis_spec_desc(16)
    ! <-- End add.
  ...
  CONTAINS
    ...
    ! added by U-MICH team. Mar 19, 2020
    if (flag_emis) then
       !ierr = pio_def_var(File, 'DO_EMIS',  pio_int, hdimids, do_emis_desc)
       ...
       do i = 1, 16
          ...
          ierr = pio_def_var(File, 'SRF_EMIS_SPEC'//num,  pio_double, hdimids, srf_emis_spec_desc(i))
            ...
            !ierr = pio_def_var(File, 'SRF_EMIS_SPEC',  pio_double, hdimids, srf_emis_spec_desc)
    end if
    ! <-- End add.
     ...
     if ( .not. adiabatic .and. .not. ideal_phys )then
        ...
        if ( flag_emis ) then
           ...
           do c= begchunk, endchunk
              ...
              !write(iulog,*) "xianwen3:ts_atm(1)=",cam_in(c)%ts_atm(1),c
           end do
           do m = 1, 16
              write(num,'(i4.4)') m
              call pio_seterrorhandling(File, PIO_BCAST_ERROR, err_handling)
              ...
              if (ierr == PIO_NOERR) then ! SRF_EMIS_SPEC variable found on restart file
                 ...
                 do c= begchunk, endchunk
                    ...
                    !write(iulog,*) "xianwen3:srf_emis_spec(1)=",cam_in(c)%srf_emis_spec(1,m),c,m
                 end do
              end if
           end do
        end if
